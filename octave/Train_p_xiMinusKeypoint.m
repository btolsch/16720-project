function [p_xiMinusKeypoint_mu, p_xiMinusKeypoint_var] = train_p_xiMinusKeypoint(aTrain, LTrain, allKeypoints, allMemberships)
    ...
    for a = 1:A
        for cj = 1:K
            n = counts(a,cj);
            if (n == 0)
                p_xiMinusKeypoint_mu(:,a,cj,:) = NaN;
                p_xiMinusKeypoint_var(:,a,cj,:,:) = NaN;
            else if (n == 1)
                p_xiMinusKeypoint_mu(:,a,cj,:) = s(:,a,cj,:);
                p_xiMinusKeypoint_var(:,a,cj,:,:) = NaN;
            else
                mu = s(:,a,cj,:) / n;
                p_xiMinusKeypoint_mu(:,a,cj,:) = mu;
                p_xiMinusKeypoint_var(:,a,cj,1,1) = (s2_11(:,a,cj) / (n-1)) - (mu(:,1) .* mu(:,1));
                p_xiMinusKeypoint_var(:,a,cj,1,2) = (s2_12(:,a,cj) / (n-1)) - (mu(:,1) .* mu(:,2));
                p_xiMinusKeypoint_var(:,a,cj,2,1) = p_xiMinusKeypoint_var(:,a,cj,1,2);
                p_xiMinusKeypoint_var(:,a,cj,2,2) = (s2_22(:,a,cj) / (n-1)) - (mu(:,2) .* mu(:,2));
            end
        end
    end
end
